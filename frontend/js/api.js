...
//const API_URL = "https://192.168.0.13:8000";
const API_URL = 'https://cloud-conta-backend.onrender.com';
...
export async function obtenerAsientos() {
  const respuesta = await fetch(`${API_URL}/asientos/`);

  return await procesarRespuesta(respuesta);
}

export async function obtenerAsiento(id) {
  const respuesta = await fetch(`${API_URL}/asientos/${id}`);
...
export async function actualizarAsiento(id, datos) {
  const respuesta = await fetch(
    `${API_URL}/asientos/${id}`,
...
export async function obtenerLibroDiario() {
  const respuesta = await fetch(`${API_URL}/libro-diario`);

  return await procesarRespuesta(respuesta);
}

export async function obtenerLibroMayor() {
  const respuesta = await fetch(`${API_URL}/libro-mayor`);

  return await procesarRespuesta(respuesta);
}

export async function obtenerBalanceSumasSaldos() {
  const respuesta = await fetch(`${API_URL}/balance-sumas-saldos`);

  return await procesarRespuesta(respuesta);
}

export async function obtenerBalanceGeneral() {
  const respuesta = await fetch(`${API_URL}/balance-general`);

  return await procesarRespuesta(respuesta);
}

export async function obtenerEstadoResultados() {
  const respuesta = await fetch(`${API_URL}/estado-resultados`);

  return await procesarRespuesta(respuesta);
}

export async function confirmarAsiento(movimientoId) {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.detail || 'No se pudo confirmar el asiento.');
  }
